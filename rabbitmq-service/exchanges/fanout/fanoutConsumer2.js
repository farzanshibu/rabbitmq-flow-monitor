...
class FanoutConsumer2 {
  constructor() {
    ...
    this.queueName = process.env.FANOUT_QUEUE_2 || 'fanout.queue.2';
    this.consumerTag = 'fanout-consumer-2';
    this.messageCount = 0;
  ...
  async setup() {
    try {
      ...
      console.log(`📥 Fanout Consumer 2 setup complete:`);
  ...
  async startConsuming() {
    if (this.isConsuming) {
      console.log('⚠️  Fanout Consumer 2 is already running');
      return;
    }
    ...
    try {
      ...
      await channel.consume(this.queueName, async (msg) => {
        ...
        try {
          ...
          console.log(`📨 [Fanout Consumer 2] [${new Date().toLocaleTimeString()}] Received broadcast #${this.messageCount}:`);
          ...
          channel.ack(msg);
          console.log(`✅ [Fanout Consumer 2] Broadcast #${this.messageCount} processed and acknowledged`);
          
        } catch (error) {
          console.error(`❌ [Fanout Consumer 2] Error processing message:`, error.message);
          
          // Reject message and requeue it
          channel.nack(msg, false, true);
          console.log(`🔄 [Fanout Consumer 2] Message rejected and requeued`);
        }
      }, {
  ...
  async processMessage(messageContent) {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        console.log(`⚙️  [Fanout Consumer 2] Processed broadcast '${messageContent.id}' in ${processingTime}ms`);
        resolve();
      }, processingTime);
    });
  }
  ...
  async stop() {
    console.log('🛑 Stopping Fanout Consumer 2...');
    try {
...
process.on('SIGINT', async () => {
  console.log('\n📡 Received SIGINT, shutting down Fanout Consumer 2 gracefully...');
  if (consumer) {
    await consumer.stop();
  }
  process.exit(0);
...
process.on('SIGTERM', async () => {
  console.log('\n📡 Received SIGTERM, shutting down Fanout Consumer 2 gracefully...');
  if (consumer) {
    await consumer.stop();
  }
  process.exit(0);
...
const consumer = new FanoutConsumer2();

if (import.meta.url === `file://${process.argv[1]}`) {
  console.log('📡 Starting Fanout Consumer 2...');
  consumer.startConsuming().catch(console.error);
}

export default FanoutConsumer2;
